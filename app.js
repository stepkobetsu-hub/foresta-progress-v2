...
async function renderTeacherStudent(studentId, { force = false } = {}) {
  ...
  $("inputLesson").insertAdjacentHTML("afterend", '<button id="correctLesson" class="ghostBtn compactCorrectionButton" type="button">次回宿題を確認・調整</button>');
  bindSelectedTabs();
...
function elementaryTopTestEntryHtml(rows){return `<article id="elementaryTopTestEntry" class="card elementaryTopTestEntry"><div><p class="cardTitle">学校の単元テスト入力</p><p class="muted">最近の学校単元テストを入力します。裏面がないテストは表面だけで保存できます。</p></div><form id="elementaryTopTestForm" class="elementaryTopTestForm" onsubmit="return false"><label>科目<select id="elementaryTopTestSubject" class="field">${rows.map(r=>`<option>${esc(r.subject)}</option>`).join('')}</select></label><label id="elementaryTopTestUnitWrap">単元（章）<select id="elementaryTopTestUnit" class="field"></select></label><label id="elementaryTopTestFreeWrap" class="hidden">単元名<input id="elementaryTopTestFree" class="field" maxlength="80"></label><label class="elementaryFrontScore">表面の点数<input id="elementaryTopTestScore" class="field" type="number" min="0" max="999" value="50" required></label><label class="elementaryFrontMax">表面の満点<input id="elementaryTopTestMax" class="field" type="number" min="1" max="999" value="100"></label><label class="elementaryBackScore">裏面の点数<input id="elementaryTopTestBackScore" class="field" type="number" min="0" max="999" value="30"></label><label class="elementaryBackMax">裏面の満点<input id="elementaryTopTestBackMax" class="field" type="number" min="1" max="999" value="50"></label><small class="elementaryBackScoreNote">※裏面がないテストは、裏面の点数・満点は空欄でかまいません。</small><label class="elementaryTopTestDate">テスト日<input id="elementaryTopTestDate" class="field" type="date" value="${dateInputValue(new Date())}"></label><button class="primaryBtn" type="submit">保存</button></form><output id="elementaryTopTestStatus" class="lessonSaveStatus" aria-live="polite"></output></article>`}
...
async function renderRangeSettings() {
  ...
  let rangeOptionsGeneration = 0;
  const update = async () => {
    const generation = ++rangeOptionsGeneration;
    const requested = { school: school.value, grade: $("rangeGrade").value, subject: $("rangeSubject").value };
    if (!school.value) return;
    test.innerHTML = '<option value="">読み込み中…</option>';
    const data = await api("getRangeOptions", requested);
    if (generation !== rangeOptionsGeneration || requested.school !== school.value || requested.grade !== $("rangeGrade").value || requested.subject !== $("rangeSubject").value) return;
...
async function openProgress(options) {
  ...
  try {
    ...
    let rangeClosing = false;
    const rangeStatus = () => $("rangeAutoSave");
    const rangeContextIsCurrent = () => !rangeMode || (
      $("rangeSchool")?.value === options.school &&
      $("rangeGrade")?.value === options.grade &&
      $("rangeSubject")?.value === options.subject &&
      $("rangeTest")?.value === options.testId &&
      $("rangeType")?.value === options.rangeType
    );
    const saveRangeSelection = async () => {
      if (!rangeMode) return true;
      if (rangeSaving) { rangeDirty = true; return; }
      clearTimeout(rangeSaveTimer);
      if (!rangeContextIsCurrent()) {
        rangeDirty = true;
        if (rangeStatus()) rangeStatus().textContent = "選択条件が変わりました・開き直してください";
        status("学校・学年・科目・テストの選択が変わりました。最新条件で進行表を開き直してください。", true);
        return false;
      }
      rangeSaving = true;
      rangeDirty = false;
      ...
      try {
        const result = await api("saveRange", { ...options, unitIds }, { silent: true });
        if (result?.queued || !Number.isInteger(result?.saved)) throw new Error("保存完了を確認できませんでした。もう一度お試しください。");
        invalidateProgressionCache(options);
    ...
    const scheduleRangeSave = () => {
      if (!rangeMode || rangeClosing) return;
      rangeDirty = true;
      clearTimeout(rangeSaveTimer);
